...
const HabitDetail = ({ habits }) => {
  ...
  // Find habit by id. Note params are strings, so parse or compare with ==
  const habit = habits.find((h) => h.id.toString() === id);

  if (!habit) {
    return (
      <div style={containerStyle}>
        <h2 style={{ color: 'var(--text-primary)' }}>Habit Not Found</h2>
        <p style={{ color: 'var(--text-secondary)', marginTop: 'var(--spacing-md)' }}>
          We couldn't find a habit with the ID: {id}.
        </p>
  ...
  return (
    <div style={containerStyle}>
      ...
      <section style={sectionStyles}>
        <h2 style={sectionTitle}>Habit Details</h2>
        <div style={detailCardStyle}>
          ...
          <div style={infoGridStyle}>
            ...
            <div style={infoItemStyle}>
              <span style={infoLabelStyle}>Current Streak</span>
              <span style={infoValueStyle}>🔥 {habit.streak} Days</span>
            </div>
          </div>
        </div>
      </section>
